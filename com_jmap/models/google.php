 ...
 * @since 3.1
 */
class JMapModelGoogle extends JMapModel {
	/**
	...
	private function getSitesProfiles($service, $client, $params) {
		...
		if (count ( $items ) != 0) {
			foreach ( $items as &$profile ) {
				...
				if ($this->purifyDomain ( $profile->getwebsiteUrl () ) == $this->purifyDomain ( $params->get ( 'ga_domain' ) )) {
	...
	public function getData() {
		...
		$client->setApplicationName ( 'JSitemap Professional' );
		...
		if ($params->get ( 'ga_api_key' ) and $params->get ( 'ga_client_id' ) and $params->get ( 'ga_client_secret' )) {
			...
			$client->setClientId ( '1229958023-v6o02cp8hj71rijdpc110efvsmjd9f9e.apps.googleusercontent.com' );
			$client->setClientSecret ( 'mkicQ8LsbYyMes2DwF6DhQ-n' );
			$client->setDeveloperKey ( 'AIzaSyBOXBjtrtYPTQmpupLwY5AhKmazQqVQPzw' );
		}
...
HTML;
	...
	($this->currentProfile->getWebsiteUrl() ? "<span class='label label-info'>" . $this->currentProfile->getWebsiteUrl() . "</span>" : null) .
	($hasOwnCredentials ? null : "<span data-content='" . JText::_('COM_JMAP_GOOGLE_APP_NOTSET_DESC') . "' class='label label-warning hasPopover google pull-right'>" . JText::_('COM_JMAP_GOOGLE_APP_NOTSET') . "</span>") .
	'<div id="ga-dash">
